...
Plot Forecast
'''
def plot_forecast(index, real, predictions, multisteps):
    ...
    plt.xlabel('Index') 
    plt.legend(['Real (last 24h)', 'Predictions (next ' + str(multisteps) + 'h)'], loc='upper left') 
    plt.show()
...
for train_index, test_index in tscv.split(X):
    ...
    early_stop = tf.keras.callbacks.EarlyStopping(monitor='val_mae', min_delta=0, patience=20, verbose=verbose, mode='auto')
...
# Forecast
predictions = forecast(best_model, X, timesteps, multisteps, scaler, columns, nr_features, df_all)
step = 1 
print('\n')
...
df_last_timesteps = df_all_until_now.tail(timesteps*2)
plot_forecast(df_last_timesteps.index.values, df_last_timesteps['speed_diff'].values, predictions, multisteps)
...
df_last_timesteps_aux = df_all_until_now.tail(timesteps)['speed_diff'].values
errors = []
