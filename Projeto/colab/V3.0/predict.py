...
# Forecast
predictions = forecast(best_model, X, timesteps, multisteps, scaler, columns, nr_features, df_scaled)
step = 1 
print('\n')
print(10*('-') + ' ' + file_path + ' ' + 10*('-'))
print('\n> Prediction for the next %d MultiSteps\n' % multisteps)
print('> speed_diff(max): %d' % df_until_now['speed_diff'].max())
print('> speed_diff(min): %d\n' % df_until_now['speed_diff'].min())
