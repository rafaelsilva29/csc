...
class PlayerAgent:
  ...
  '''
  def play_the_game(self, train_flag):
    #build training set
    if train_flag:
        print("Build train_set...")
        training_set = self.build_training_set()
    else:
        print("Get best train_set...")
        training_set = np.load('training_data.npy')

    mlp = MLP() 
    
    model = mlp.build()
    
    X, y = mlp.fit(training_set)
    ...
    for game in range(self.nr_of_games_for_training):
        ...
        #the game is finished. Was it a decent game?
        if (max(scores) > self.score_requirement) and (score >= max(scores)):
            for play in game_memory:
                if play[0] == 0:
                    one_hot_action = [1, 0]
                elif play[0] == 1:
            ...
            training_data.append([one_hot_action, play[1]])
